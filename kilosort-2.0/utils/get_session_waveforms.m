function results = get_session_waveforms(dataDir, ksDir, only_good)
    % params
    nWF = 1e3; % number of waveforms to get for each unit
    spkOffset = 1e-3; % 1.5e-3; seconds before spike time to get
    spkDur = 4e-3; % seconds after spike time to get
    spkDurSamples = spkDur*30e3; % total samples to pre-allocate for waveform
...
%         ind = cluster_labels>0 & cluster_labels<3;
    ...
    maxTime = double(max(spkSamp))/30e3;
    
    % read in spikes for all units
    for thisUnit=1:numUnits
        curUnitID = goodIDs(thisUnit);
        curSpikeTimes = double(spkSamp(sID==curUnitID))/30e3;
        ...
        if ~isempty(curSpikeTimes)
            ...
            curSpikeTimes((curSpikeTimes+spkDur*10)*30e3 > nSamp) = []; % spikes extending past recording
            curSpikeTimes((curSpikeTimes-spkOffset*10)*30e3 < 1) = []; % spikes starting before recording
            ...
            mean_wave_filt = getSpikeWaveform(mmf, ...
                spikeTimeKeeps, 30e3, spkOffset, spkDur);
            waveFormsMean(:,:,thisUnit) = mean_wave_filt;
        end
            disp(['Completed ' int2str(thisUnit) ' units of ' int2str(numUnits) '.']);
    end
    
    % Get top PC of waveforms across channels, that that as the waveform
    for i=1:numUnits
...
%     end
    ...
    results = struct('mxWF',mxWF,'mxWF_d',mxWF_d,'meanRate',meanRate,...
