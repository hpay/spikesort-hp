...
% Remove bad channels - both manually specified and low firing rate
igood = true(size(chanMap));
if ~isempty(getOr(ops, 'badChannels', []))    
    badChannels = ops.badChannels;
    fprintf('Manually excluding channels: %s  \n', num2str(badChannels));    
...
if getOr(ops, 'minfr_goodchannels', .1)>0 % discard channels that have very few spikes
    ...
    igood_minfr = get_good_channels(ops, chanMap);
    igood = igood & igood_minfr(:);
end
