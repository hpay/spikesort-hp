...
ops = rez.ops;
% dt = 1/1000;
dt = 0.5/1e3; % use finer steps for CCG binning, following SNC's observations
...
for j = 1:Nk
    ...
    while Th>=ops.Th(2)
      % continually lower the threshold, while the estimated unit contamination is low
        ...
        [K, Qi, Q00, Q01, rir] = ccg(st, st, 1e3, dt); % % compute the auto-correlogram with 500 bins at 1ms bins - EDIT HP following SC, to 1000 bins from 500
    ...
    st = ss(vexp>Th); % take spikes above the current threshold
    [K, Qi, Q00, Q01, rir] = ccg(st, st, 1e3, dt); % % compute the auto-correlogram with 500 bins at 1ms bins % EDIT HP following SC
    Q = min(Qi/(max(Q00, Q01))); % this is a measure of refractoriness
